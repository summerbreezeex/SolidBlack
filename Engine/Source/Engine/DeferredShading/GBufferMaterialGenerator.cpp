...
GBufferMaterialGenerator::GBufferMaterialGenerator() {
    vertexShaderMask = 0x00FFFF00;
    fragmentShaderMask = 0x0000FFFF;
    materialMask = 0xFF00FFFF;
    materialBaseName = "DeferredShading/GBuffer/";
    impl = new GBufferMaterialGeneratorImpl(materialBaseName);
}
...
Ogre::GpuProgramPtr GBufferMaterialGeneratorImpl::generateVertexShader(MaterialGenerator::Perm permutation) {
    ...
    Ogre::String programName = baseName + "VP_" + Ogre::StringConverter::toString(permutation);

    // Create shader object
    Ogre::HighLevelGpuProgramPtr ptrProgram =
        Ogre::HighLevelGpuProgramManager::getSingleton().createProgram(programName, Ogre::ResourceGroupManager::DEFAULT_RESOURCE_GROUP_NAME, "cg", Ogre::GPT_VERTEX_PROGRAM);
...
Ogre::GpuProgramPtr GBufferMaterialGeneratorImpl::generateFragmentShader(MaterialGenerator::Perm permutation) {
    ...
    Ogre::String programSource = ss.str();
    Ogre::String programName = baseName + "FP_" + Ogre::StringConverter::toString(permutation);

    // Create shader object
    Ogre::HighLevelGpuProgramPtr ptrProgram =
        Ogre::HighLevelGpuProgramManager::getSingleton().createProgram(programName, Ogre::ResourceGroupManager::DEFAULT_RESOURCE_GROUP_NAME, "cg", Ogre::GPT_FRAGMENT_PROGRAM);
    ptrProgram->setSource(programSource);
...
Ogre::MaterialPtr GBufferMaterialGeneratorImpl::generateTemplateMaterial(MaterialGenerator::Perm permutation) {
    Ogre::String matName = baseName + "Mat_" + Ogre::StringConverter::toString(permutation);
    ...
    pass->setName(baseName + "Pass_" + Ogre::StringConverter::toString(permutation));
